...
const Index = () => {
  ...
  const getPokemones = async(o) =>{
   ...
   const getAllPokemones = async(o) =>{
    const liga = 'https://pokeapi.co/api/v2/pokemon?limit=100000&offset='+o;
    axios.get(liga).then( async(response)=>{
      const respuesta = response.data;
  ...
  return (
    <Container className="shadow bg-danger mt-3">
      <Row className="animate__animated animate__bounceInDown">
        <Col>
        <InputGroup className=" animate__fadeInTopLeftmt-3 mb-1 shadow">
      ...
      <Row className="mt-3">
        ...
        {listado.length == 0 ?<Col  className="text-center fs-2 mb-3">No hay coincidencia</Col> : ''}
